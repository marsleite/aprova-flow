    ...
    return (
        <Card variant={variant} padding="lg" className="flex flex-col justify-between min-h-[160px] transition-all duration-300 hover:bg-am-surface-elevated group">
            <div className="flex items-center justify-between mb-4">
                <span className="text-[11px] font-semibold uppercase tracking-widest text-am-text-secondary font-mono">{title}</span>
            ...
            <div className="mt-auto">
                {loading ? (
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        {delta && (
                            <span className={`flex items-center gap-1 text-[11px] font-mono mt-2 ${delta.trend === 'up' ? 'text-am-success' : delta.trend === 'down' ? 'text-am-error' : 'text-am-text-tertiary'
                                }`}>
                                {delta.trend === 'up' && '+'}
                                {delta.trend === 'down' && '-'}
                                {delta.value}%
