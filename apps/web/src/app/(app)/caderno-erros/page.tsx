...
const DIMENSION_LABELS: Record<string, { label: string; color: string; emoji: string }> = {
    legislacao: { label: 'Legislação', color: 'text-primary', emoji: '📜' },
    jurisprudencia: { label: 'Jurisprudência', color: 'text-primary', emoji: '⚖️' },
    interpretacao: { label: 'Interpretação', color: 'text-am-warning', emoji: '🔍' },
    conceitual: { label: 'Conceitual', color: 'text-green-500', emoji: '📚' },
};

function SeverityBar({ value }: { value: number }) {
    const color = value >= 8 ? 'bg-am-error' : value >= 5 ? 'bg-am-warning' : 'bg-green-500';
    return (
        <div className="flex items-center gap-2">
            <div className="flex-1 h-2 bg-muted/50 rounded-am-full overflow-hidden">
                <div className={`h-full rounded-am-full ${color} transition-all duration-500`} style={{ width: `${value * 10}%` }} />
            </div>
            <span className="text-[10px] text-muted-foreground w-6 text-right font-mono">{value}</span>
        </div>
    );
}

function ScoreBar({ label, emoji, value, color }: { label: string; emoji: string; value: number; color: string }) {
    const barColor = value >= 70 ? 'bg-green-500' : value >= 40 ? 'bg-am-warning' : 'bg-am-error';
    return (
        <div className="space-y-1">
            ...
            <div className="h-1.5 bg-muted/50 rounded-am-full overflow-hidden">
...
function FlashcardItem({ card }: { card: Flashcard }) {
    ...
    return (
        <div
            ...
            className="cursor-pointer rounded-md border border-am-success/20 bg-green-500/5 p-4 min-h-[120px] flex flex-col justify-between hover:border-am-success/40 transition-colors shadow-am-sm"
        >
            <div>
                <p className="text-[10px] font-bold font-mono uppercase tracking-wider text-green-500 mb-2">{card.topic}</p>
                {flipped ? (
                    <p className="text-am-body-sm text-foreground font-medium">{card.back}</p>
                ) : (
                    <p className="text-am-body-sm text-muted-foreground">{card.front}</p>
                )}
            </div>
        ...
        <div className="flex flex-col gap-8 pb-10">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-4 px-6 border-b border-border bg-card/30 backdrop-blur-md">
                <div>
                    ...
                    <p className="text-am-caption text-muted-foreground mt-1 max-w-lg">
            ...
            <div className="px-6 space-y-6">
                {loading ? (
                    ...
                    <>
                        ...
                        {/* Filters & Actions */}
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 bg-muted p-3 rounded-md border border-border">
                            <div className="flex items-center gap-2 flex-grow">
                                <Filter className="h-4 w-4 text-muted-foreground ml-2" />
                                <select
                                    value={filterMateria}
                                    onChange={(e) => setFilterMateria(e.target.value)}
                                    className="bg-transparent border-none text-am-body-sm text-foreground font-medium focus:outline-none w-full sm:w-auto min-w-[200px]"
                                >
                                    <option value="" className="bg-muted">Todas as matérias</option>
                                    {allMaterias.map(m => <option className="bg-muted" key={m} value={m}>{m}</option>)}
                                </select>
                            </div>
                        ...
                        {/* Diagnosis Result */}
                        {diagnosisError && (
                            <div className="p-4 bg-am-error/10 border border-am-error/30 rounded-md text-am-body-sm text-am-error">
                                {diagnosisError}
                            </div>
                        )}

                        {canUseGapAnalyzer && diagnosis && (
                            <div className="rounded-xl border border-border/50/40 bg-card p-6 space-y-6 relative overflow-hidden shadow-am-lg" style={{ background: 'linear-gradient(145deg, var(--color-am-surface) 0%, rgba(139,92,246,0.05) 100%)' }}>
                                ...
                                <div className="flex items-center justify-between relative z-10">
                                    <h3 className="flex items-center gap-2 font-sans text-am-h5 font-bold text-foreground">
                                ...
                                {diagnosisSample && (
                                    <div className="relative z-10 rounded-md border border-border/50/30 bg-muted px-4 py-3">
                                        <p className="text-am-caption text-muted-foreground">
                                            Base desta leitura: <span className="font-medium text-foreground">{diagnosisSample.analyzedCount}</span> de{' '}
                                            <span className="font-medium text-foreground">{diagnosisSample.availableCount}</span> erros filtrados.
                                        </p>
                                    </div>
                                )}
                                ...
                                {/* Summary */}
                                {diagnosis.summary && (
                                    <p className="text-am-body-sm text-muted-foreground bg-muted rounded-md p-4 border border-am-border-subtle relative z-10">
                                        {diagnosis.summary}
                                    </p>
                                )}
                                                ...
                                                return (
                                                    <div key={i} className="rounded-md border border-border bg-card p-4 space-y-3">
                                                        <div className="flex items-start justify-between gap-3">
                                                            <span className="text-am-body-sm text-foreground">{gap.description}</span>
                                                        ...
                                                        <div className="flex items-start gap-2 mt-2 bg-am-warning/5 p-2.5 rounded border border-am-warning/10">
                                                            ...
                                                            <p className="text-am-caption text-muted-foreground leading-relaxed">{gap.advice}</p>
                                                        </div>
                                                    </div>
                                                );
                                            })
                                            ...
                                            diagnosis.patterns.map((p, i) => (
                                                <div key={i} className="rounded-md border border-border bg-card p-4">
                                                    <div className="flex items-start gap-2">
                                                        ...
                                                        <span className="text-am-body-sm text-muted-foreground">{p}</span>
                                                    </div>
                                                </div>
                                            ))
                                        ) : null}
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        {diagnosis.overallScore && (
                                            <div className="rounded-md border border-border bg-card p-5 space-y-4">
                                                <h4 className="text-am-caption font-bold font-mono text-foreground uppercase tracking-wider">
                                                    Perfil de Retenção
                                                </h4>
                                        ...
                                        {diagnosis.criticalSubjects.length > 0 && (
                                            <div className="rounded-md border border-am-warning/20 bg-am-warning/5 p-5">
                                ...
                                {diagnosis.flashcards && diagnosis.flashcards.length > 0 && (
                                    <div className="space-y-4 relative z-10 pt-4 border-t border-border">
                                        <h4 className="flex items-center gap-2 text-am-caption font-bold font-mono text-green-500 uppercase tracking-wider">
                        ...
                        {totalErrors === 0 ? (
                            <div className="text-center py-20 bg-card rounded-xl border border-border shadow-am-md">
                                <div className="mx-auto w-16 h-16 rounded-am-full bg-green-500/10 flex items-center justify-center mb-4">
                            ...
                            <div className="space-y-6">
                                {sortedGroups.map(([materia, entries]) => (
                                    <div key={materia} className="rounded-xl border border-border bg-card shadow-am-sm overflow-hidden">
                                        {/* Group Header */}
                                        <div className="flex items-center justify-between px-5 py-3 bg-muted border-b border-border">
                                            <div className="flex items-center gap-3">
                                                <Badge variant="error" className="h-7 w-7 p-0 flex items-center justify-center rounded-md">
                                                    {entries.length}
                                                </Badge>
                                                <h3 className="font-sans text-am-body font-bold text-foreground">{materia}</h3>
                                            </div>
                                        </div>
                                        ...
                                        <div className="divide-y divide-border">
                                            {entries.map(({ attempt, question }) => {
                                                ...
                                                return (
                                                    <div key={attempt.id} className="px-5 py-4 transition-colors hover:bg-card">
                                                        {/* Collapsed Row */}
                                                        <div
                                                        ...
                                                        >
                                                            <div className="flex-1 min-w-0">
                                                                <p className="text-am-body-sm text-foreground line-clamp-2 leading-relaxed">
                                                                    {question.statement}
                                                                </p>
                                                                <div className="flex items-center gap-3 mt-3 flex-wrap">
                                                                    ...
                                                                    <span className="text-am-caption text-green-500 bg-green-500/10 px-2 py-0.5 rounded">
                                                        ...
                                                        {isExpanded && (
                                                            <div className="mt-5 space-y-4 border-t border-border pt-4">
                                                                {/* Full Statement */}
                                                                <div className="bg-muted rounded-md p-4 border border-am-border-subtle">
                                                                    <p className="text-am-body-sm text-foreground whitespace-pre-wrap leading-relaxed">{question.statement}</p>
                                                                </div>
                                                                ...
                                                                <div className="space-y-2">
                                                                    {question.alternatives.map(alt => {
                                                                        ...
                                                                        let wrapperClass = "bg-muted border-am-border-subtle text-muted-foreground";
                                                                        let keyClass = "bg-muted text-muted-foreground";

                                                                        if (isCorrect) {
                                                                            wrapperClass = "bg-green-500/10 border-am-success/30 text-green-500";
                                                                            keyClass = "bg-green-500/20 text-green-500";
                                                                        ...
                                                                        return (
                                                                            <div
                                                                                key={alt.key}
                                                                                className={`flex items-start gap-3 p-3 rounded-md border text-am-body-sm ${wrapperClass}`}
                                                                            >
                                                                ...
                                                                <div className="flex flex-wrap items-center gap-3 pt-2">
                                                                    {!attempt.mastered ? (
                                                                        <Button
                                                                            ...
                                                                            size="sm"
                                                                            className="bg-green-500/20 hover:bg-green-500/30 text-green-500 border border-am-success/30"
                                                                        >
